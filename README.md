...
Think _"ChatGPT, but it only knows your company's documents and never makes things up."_

---

## Status

🟢 **Phase 0 complete — monorepo scaffolded and running.**
`apps/web` (React) and `apps/api` (Express) both start locally and the web app successfully calls the
API's `/api/health`. Next: **Phase 1** (auth + database). See [docs/05-roadmap.md](docs/05-roadmap.md).
...
| [docs/03-tech-stack.md](docs/03-tech-stack.md)         | Every tech choice and _why_ (interview-ready rationale)                   |
...
# 3. Run both apps together (api on :3000, web on :5173)
pnpm dev
# or individually:
pnpm --filter api dev
pnpm --filter web dev
...
Then open http://localhost:5173 — you should see the DocPilot page showing the API health JSON.
